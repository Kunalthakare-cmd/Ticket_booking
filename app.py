...
# Utility: Load and save JSON data
def load_data():
...
def book_ticket():
    try:
        ...
        # Check suspicious (more than 3 bookings)
        booking_count = sum(1 for b in all_bookings if b['fingerprint'] == data['fingerprint'])
        is_suspicious = booking_count > 3
...
def get_analytics():
    try:
        ...
        fingerprint_counts = Counter(b['fingerprint'] for b in all_bookings)
        suspicious_fingerprints = {fp: count for fp, count in fingerprint_counts.items() if count > 3}
        ...
        for fp, count in fingerprint_counts.items():
            ...
            fingerprint_details.append({
                ...
                'names_used': names,
                'routes': routes,
                'last_booking': max(b['timestamp'] for b in fp_bookings),
                'bookings': [
                    {
                        'name': b['name'],
                        'route': f"{b['source']} → {b['destination']}",
        ...
        for b in all_bookings:
            hour = b['timestamp'][:13]  # YYYY-MM-DDTHH
            timeline_data[hour] = timeline_data.get(hour, 0) + 1

        timeline = sorted([{'time': k, 'count': v} for k, v in timeline_data.items()], key=lambda x: x['time'])
